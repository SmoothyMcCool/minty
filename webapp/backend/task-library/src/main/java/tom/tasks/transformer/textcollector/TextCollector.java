package tom.tasks.transformer.textcollector;
...
public class TextCollector implements MintyTask {
	...
	private static final Logger logger = LogManager.getLogger(TextCollector.class);
	...
	public boolean giveInput(int inputNum, Packet dataPacket) {
		if (inputNum != 0) {
			logger.warn("Workflow misconfiguration detect. TextCollector should only ever have exactly one input!");
		}
		result = result + "\n\n" + dataPacket.getText();
		return true;
	}
	...
	public void setOutputConnectors(List<? extends OutputPort> outputs) {
		if (outputs.size() != 0) {
			logger.warn("Workflow misconfiguration detect. TextCollector should have no outputs!");
		}
	}
