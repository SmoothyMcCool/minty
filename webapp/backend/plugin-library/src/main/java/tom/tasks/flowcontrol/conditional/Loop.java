	...
	public TaskSpec getSpecification() {
		return new TaskSpec() {
			...
			public String produces() {
				return "Each input is sent unmodified to output 0 if the loop should continue, or 1 if the loop should end. Once a packet is sent on output 1, the task terminates.";
			}
			...
			public TaskConfigSpec taskConfiguration() {
				return new LoopConfig(Map.of(LoopConfig.BranchExpression, ""));
			}

			@Override
			public TaskConfigSpec taskConfiguration(Map<String, Object> configuration) {
				return new LoopConfig(configuration);
			}
