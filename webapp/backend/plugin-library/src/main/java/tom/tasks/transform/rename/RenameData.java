...
public class RenameData implements MintyTask {
	...
	public RenameData(RenameDataConfig config) {
		this();
		this.config = config;
	}
	...
	public boolean giveInput(int inputNum, Packet dataPacket) {
		if (inputNum != 0) {
			...
			throw new RuntimeException(
					"Workflow misconfiguration detect. RenameData should only ever have exactly one input!");
		}
		input = dataPacket;
	...
	public TaskSpec getSpecification() {
		return new TaskSpec() {
			...
			public TaskConfigSpec taskConfiguration() {
				return new RenameDataConfig();
			}

			@Override
			public TaskConfigSpec taskConfiguration(Map<String, Object> configuration) {
				return new RenameDataConfig(configuration);
			}
