...
@RunnableTask
public class SimpleTransformer implements MintyTask {

	private List<? extends OutputPort> outputs;

	private TaskLogger logger;
	private SimpleTransformerConfig config;
	...
	public SimpleTransformer(SimpleTransformerConfig config) {
		this();
		this.config = config;
	}
	...
	public void run() {
		...
		try {
			...
			String error = "SimpleTransformer caught exception while trying to apply SpEL expression: " + expression
	...
	public boolean giveInput(int inputNum, Packet dataPacket) {
		if (inputNum != 0) {
			...
			throw new RuntimeException(
					"Workflow misconfiguration detect. SimpleTransformer should only ever have exactly one input!");
		}
		input = dataPacket;
	...
	public TaskSpec getSpecification() {
		return new TaskSpec() {
			...
			public TaskConfigSpec taskConfiguration() {
				return new SimpleTransformerConfig();
			}

			@Override
			public TaskConfigSpec taskConfiguration(Map<String, Object> configuration) {
				return new SimpleTransformerConfig(configuration);
			}
