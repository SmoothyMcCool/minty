...
public class AiTaskWrapper implements Runnable {
	...
	public AiTaskWrapper(AiTask wrappedTask, OutputTask outputTask) {
	...
	public void run() {
		try {
			...
			List<AiTask> children = wrappedTask.runTask();
			Objects.requireNonNull(children);

			if (children.size() > 0) {
				wrappedChildren = new ArrayList<>();
				for (AiTask child : children) {
					AiTaskWrapper wrappedChild = new AiTaskWrapper(child, new NullOutput());
	...
	public OutputTask getOutputRenderer() {
		return outputTask;
	}
