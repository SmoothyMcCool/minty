...
public class AssistantQueryServiceImpl implements AssistantQueryService {
	...
	private final LlmRequestMetricsService llmRequestMetricsService;
	...
	public AssistantQueryServiceImpl(AssistantManagementService assistantManagementService,
			...
			LlmRequestMetricsService llmRequestMetricsService, PlatformTransactionManager transactionManager,
		...
		this.llmRequestMetricsService = llmRequestMetricsService;
	...
	private String askInternal(LlmRequest request, LlmRequestId llmRequestId) {
		...
		try {
			llmRequestMetricsService.markDequeued(llmRequestId);
	...
	public boolean cancelRequest(UserId userId, ConversationId conversationId) {
		List<RequestSummary> requests = llmRequestMetricsService.getActiveRequestsForUserAndConversation(userId,
				conversationId);
		...
		llmRequestMetricsService.markFailed(requests.get(0).llmRequestId(), "cancelled");
	...
	private String runSingleLlmCallStreaming(UserId userId, AssistantQuery query, LlmRequestId llmRequestId,
		...
		try {
			...
			Map<String, Object> params = Map.of(ToolExecutionContext.ASSISTANT_ID,
			...
			responses.publishOn(Schedulers.immediate()).doOnNext(chatClientResponse -> {
				...
				if (firstToken.get()) {
					llmRequestMetricsService.recordFirstToken(llmRequestId);
					firstToken.set(false);
				}

				@SuppressWarnings("unchecked")
				List<String> currentToolCalls = (List<String>) ToolExecutionContext.get(contextKey)
						.getOrDefault(ToolExecutionContext.ACCUMULATED_TOOL_CALLS, List.of());

				List<String> newEntries = currentToolCalls.size() > toolCalls.size()
						? new ArrayList<>(currentToolCalls.subList(toolCalls.size(), currentToolCalls.size()))
						: List.of();
				...
				if (!failed.get()) {
					...
					llmRequestMetricsService.markCompleted(llmRequestId);
					llmRequestMetricsService.recordTokenCounts(llmRequestId, usage.get().getPromptTokens(),
	...
	public String askStreamingDirect(UserId userId, AssistantQuery query, StreamResult sr) {
		LlmRequestId llmRequestId = llmRequestMetricsService.registerRequest(userId, query.getConversationId());
		llmRequestMetricsService.markDequeued(llmRequestId);
