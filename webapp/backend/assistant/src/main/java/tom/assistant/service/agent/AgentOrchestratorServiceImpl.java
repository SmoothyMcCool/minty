...
public class AgentOrchestratorServiceImpl implements AgentOrchestratorService {
	...
	private StepResult runAction(UserId userId, AssistantQuery query, AgentStep step, PlanState state, StreamResult sr)
		...
		sr.addChunk("[" + state.currentStep().left().getName() + "]Query: " + agentQuery.query().getQuery(),
				ChunkType.INTERNAL);
	...
	private boolean handleResult(UserId userId, AssistantQuery query, PlanState state, StepResult result,
		...
		case UNSTRUCTURED -> {
			...
			stepState.setStatus(LlmStatus.SUCCESS);
			if (state.currentStep().left().getVisibility() == AgentResponseVisibility.INTERNAL) {
				sr.addChunk("[" + state.currentStep().left().getName() + "]" + result.getFallbackText(),
						ChunkType.INTERNAL);
			} else {
				// User-facing message. Add it to chat history.
	...
	private void statusMessage(StreamResult sr, String msg) {
		sr.addChunk(msg, ChunkType.STATUS);
	}
}