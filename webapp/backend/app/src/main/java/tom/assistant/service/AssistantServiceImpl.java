...
public class AssistantServiceImpl implements AssistantService {
	...
	public String ask(int userId, AssistantQuery query) {
		...
		if (query.getConversationId() == null || query.getConversationId().compareTo("default") == 0) {
			...
			if (assistant == null) {
				logger.warn("Tried to build a chat session from an assistant " + 
					"that user doesn't have access to. User: " + userId + ", assistant: " + query.getAssistantId());
				return null;
			}
			...
			if (assistant.numFiles() > 0) {
				SearchRequest searchRequest = SearchRequest.builder()
						...
						.filterExpression(" assistantId == " + query.getAssistantId())
						.topK(MaxResults)
						.build();
